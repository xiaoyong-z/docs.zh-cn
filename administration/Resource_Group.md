...
自 2.2.0 版本起，StarRocks 支持资源隔离【公测中】。资源隔离功能可以限制查询任务对计算资源的消耗，让不同租户的查询任务在同一集群执行时，既能实现资源隔离，又能合理使用资源。

通过创建资源组（Resource Group），用户可以将 BE 节点的计算资源（CPU、内存）划分成若干个资源组，并且为每个资源组会关联一个或多个分类器（Classifier）。通过在分类器中设置条件（用户名、用户所属的 Role、IP 地址、 查询类型），系统能够匹配查询任务的对应信息。当用户发起查询任务时，分类器会根据查询任务的信息（用户名、用户所属的 Role、IP 地址、 查询类型），进行匹配。匹配度最高的分类器才会生效，最终该分类器所属资源组为查询任务的资源组。
...
用户可以通过创建资源组并设置相应分类器为不同查询任务隔离计算资源。
